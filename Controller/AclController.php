...
class AclController extends AclManagerAppController {
	...
	public function drop() {
		...
		$this->Session->setFlash( "Ambos ACOs y AROs han sido elimindados", 'flash', array( 'class' => 'notice' ) );
	...
	public function drop_perms() {
		if ($this->Acl->Aro->Permission->deleteAll(array("1 = 1"))) {
			$this->Session->setFlash( "Permisos eliminados", 'flash', array( 'class' => 'notice' ) );
		} else {
			$this->Session->setFlash( "Error al intentar eliminar los permisos", 'flash', array( 'class' => 'notice' ) );
		}
		$this->redirect(array("action" => "index"));
	}
	...
	public function update_acos() {
		...
		$this->Session->setFlash( $count." ACOs han sido creados/actualizados", 'flash', array( 'class' => 'notice' ) );
	...
	public function update_aros() {
		...
		$this->Session->setFlash( $count." AROs han sido creados", 'flash', array( 'class' => 'notice' ) );
		$this->redirect($this->request->referer());
	}
